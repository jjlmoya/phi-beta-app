    ...
    <div class="card" :class="owned ? 'is-owned' : 'is-not-owned'">
        ...
        <div class="card__months">
          <div  v-for="(month, index) in commonMonths" :key="month().id">
            <div class="month__wrap" :class="getMonthState(month().id, index).modifier">
                <img :src="`${getMonthState(month().id, index).image}`">
                <span>{{ month().shortName }}</span>
            </div>
          </div>
        </div>
        <div class="card__info">
            <a @click="open(`${domain}${pathname}`)" target="_blank">
...
export default {
  ...
  methods: {
    ...
    getMonthState (activeMonth, index) {
      ...
      return {
        image: match ? match.image : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==',
        modifier: `${(match ? match.type : '')} ${isActive}`
      }
    }
  }
}
